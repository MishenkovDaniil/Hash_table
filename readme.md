...
Мы реализовали и использовали так называемую хэш-таблицу с цепочками(в рамках исследования операция удаления не была реализована, так как в ней не было необходимости), представляющую собой массив списков.
На вход подавались данные произвольного значения и длины, а именно слова из "Войны и Мира" Льва Толстого. 

В рамках исследования распределения на вход каждой функции было подано 24709 уникальных слова, а результат их выполнения был взят по модулю количества списков внутри `хэш-таблицы`, равному 4999. В ходе работы со строками были рассмотрены следующие хэш-функции:
...
Последние 2 `хэш-функции` (см. рис. 4) показали наиболее хорошие распределения. При этом  у `crc-64` наиболее равномерный график, где количество элементов на значение не превышает 10 практически нигде. У функции `hash_rotl` большая часть графика также является равномерной, но все же присутствуют нередкие отклонения от среднего значения (в 5 элементов) в 3 - 4 раза и более.
...
Поскольку хэш-таблицы используются для быстрого поиска среди большого количеству данных, то даже при использовании хорошей хэш-функции необходимо оптимизировать работу хэш-таблицы во времязатратных местах. 
...
- Мы также провели "нулевую оптимизацию" программы, преобразовав все строки из `char *` в `char [32]` (предварительно выяснив что длина слова не превышает 25 символов) для более быстрого обращения к ним за счет их нахождения в кэше и для будущего применения этого факта в наших оптимизациях.
...
Проанализировав работу самой времязатратной на данный момент функции `hash_find` можно понять, что почти все время занимает линейный поиск по элементам списка. Ассемблерный код цикла после компиляции программы представлен на рис. 8. Видно, что при использовании флага -O0, этот цикл занимает большое количество кода и создает множество меток, перемещения между которыми заметно замедляют работу поиска в цикле. Для оптимизации небольшой части функции мы выбрали оптимизацию с помощью ассемблерной вставки: мы переписали код цикла, учитывая логику его работы и тем самым сократили количество прыжков по меткам и значительно сократили количество обращений к памяти (рис.9). Увидеть формальный результат можно применив профайлер к новой программе (рис.10): функция стала занимать 29% времени работы программы вместо прежних 36%.

|![](/data/dump_data/godbolt_hash_find.png?raw=true)      |
|:-------------------------------------------------------:|
|*Рис.8 ассемблерный код цикла поиска функции `hash_find`.*|

|![](/data/dump_data/asm_inline_code.png?raw=true)                   |
|:------------------------------------------------------------------:|
|*Рис.9 код цикла поиска функции `hash_find` с ассемблерной вставкой.*|
...
Как видно из таблицы, первые две оптимизации ускорили программу относительно предыдущей практически вдвое, последняя чуть меньше, на 30 процентов.
По сравнению с начальной версией программы мы получили ускорение практически в пять раз, что является очень хорошим результатом.
...
- В нашем случае благодаря оптимизациям нам удалось ускорить программу практически в 5 раз. Следовательно, такие оптимизации действительно полезны в случае слабой оптимизации компилятором.
