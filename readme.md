...
|![](/data/histograms/histogram_of_1_func.png?raw=true) | ![](/data/histograms/for_my_dude.png?raw=true) |
...
- По мере проведения оптимизаций мы будем заполнять таблицу, показывающую ускорение абсолютного времени работы программы, которая представлена ниже (подробности измерений будут в разделе "обсуждение").
...
Проанализировав работу самой времязатратной на данный момент функции `hash_find` можно понять, что почти все время занимает линейный поиск по элементам списка. Чтобы ускорить программы мы с помощью ассемблерной вставки заинлайним нашу функции сравнения строк, тем самым избавившись от долгих операций `call` и `ret`.

С помощью программы `godbolt` удостоверимся в том что наша функция станет работать быстрее: рассмотрим ассемблерный код функции поиска до оптимизации (рис. 8) и после нее (рис. 9). Мы видим, что структура функции не ухудшилась после вставки, мы смогли уменьшить количество пересылок между регистрами за счет появления возможности хранения строк в любых регистрах. Более того, вставка привела к лучшей оптимизации встроенным компилятором, так как теперь при выходе из программы производится на один `pop` меньше (что значимо, ведь эта операция относительно долгая).
...
Так как относительно прежней версии программы мы получили ускорение всего на 4%, то проведение последующих оптимизаций будет нецелесообразно. 
...
Как видно из таблицы, первые две оптимизации ускорили программу относительно предыдущей довольно сильно, на 82 и 30 процентов соответственно, последняя ускорила предыдущую версию всего на 4%, что на самом деле является ожидаемым результатом, ведь оптимизация не изменяла алгоритм работы в корне, а лишь убрала несколько ассемблерных команд из кода. По сравнению с начальной версией программы мы получили ускорение практически в два с половиной раза, что является хорошим результатом.
