...
Мы реализовали и использовали так называемую хэш-таблицу с цепочками(в рамках исследования операция удаления не была реализована, так как в ней не было необходимости), представляющую собой массив списков, в который помещали данные произвольного значения и длины, а именно слова из "Войны и Мира" Льва Толстого. 
...
По полученным после распределения слов хэш-функциями данным были построены графики.
...
4-ая и 5-ая `хэш-функции` (см. рис. 3) распределяют строки заметно лучше, чем ранее рассмотренные: хоть отклонения от среднего значения в ~50 значений на лист являются частыми, но все они небольшие относительно ранее увиденных, а именно, максимальное отклонение - примерно в 2 раза.
