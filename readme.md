...
Последние 2 `хэш-функции` (см. рис. 4) показали наиболее хорошие распределения. При этом  у `crc-64` наиболее равномерный график, где практически все значения лежат в диапазоне от 0 до 10. У функции `hash_rotl` большая часть графика также является равномерной, но все же присутствуют нередкие отклонения от среднего значения (в 5 элементов) в 3 - 4 раза и более.
...
| С оптимизированной функцией поиска     | 179.9 ± 1.5      | 3.61                                    |1.96                                      |
| С оптимизированной функцией `strcmp`   | 137.7 ± 3.6      | 4.72                                    |1.31                                      |
